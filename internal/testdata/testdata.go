...
func DCMPath(name string) string {
	return filepath.Join(Root(), "dcm", name)
}

func RequireDCMFrame(t *testing.T, dcmName string) ([]byte, DCMFrameMeta) {
	t.Helper()
	base := DCMPath(dcmName)
